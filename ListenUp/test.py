...
def edit_debate():
    ...
    if request.method == 'POST' and form.validate():
        arg=singleArgument.query.filter_by(arguments_id=request.args.get('argument_id')).first()
        arg.content=form.content.data
        db.session.commit()
        return redirect(url_for('view_debate'))
...
def view_debate():
    if request.args.get('likes')== "pressed" or request.args.get('dislikes') == "pressed":
        ...
        return render_template('view_debate.html', title=request.args.get('title'),
                               arguments_id=request.args.get('argument_id'), content=request.args.get('content'), curr=current_user,
                               subcontent=request.args.get('subcontent'), author=request.args.get('author'),author_id=curr_id,
                               right_args=right_args, left_args=left_args, left_len=left_len, right_len=right_len, likes = arg.likes,dislikes=arg.dislikes)
    else:
        ...
        if curr is None and request.args.get('author_id') != curr_id:
            ...
            if  request.method == 'POST' and form.validate():
                ...
                return render_template('view_debate.html', title=request.args.get('title'),
                                   ...
                                   content=request.args.get('content'), author=request.args.get('author'), curr=current_user,
                ...
                left_args = singleArgument.query.filter_by(arguments_id=request.args.get('argument_id')).order_by(
                    singleArgument.likes.desc())
                return render_template('expand_debate.html', title=request.args.get('title'), arguments_id=request.args.get('argument_id'), curr=current_user,
            ...
            #app.logger.info(author_id)
            #app.logger.info("IM TIRED "+str(right_args.first().content))
            right_args = singleArgument.query.filter_by(arguments_id=request.args.get('argument_id')).order_by(singleArgument.likes.desc())
            left_args = singleArgument.query.filter_by(arguments_id=request.args.get('argument_id')).order_by(singleArgument.likes.desc())
            left_len = 0
            right_len = 0
            for arg in left_args:
                if arg.agree_or_disagree == False:
            ...
            return render_template('view_debate.html', title=request.args.get('title'), curr=current_user,
                               arguments_id=request.args.get('argument_id'),
                               content=request.args.get('content'), author=request.args.get('author'), author_id=author_id,
